...
instance Foldable AlgebraicGraph where
    ...
    foldr f acc graph = go acc graph  -- reducere graf parcurg toate nodurile si le combin
      where 
        go acc Empty= acc
        go acc (Node x)=f x acc
        go acc (Overlay l r)=foldr f (foldr f acc r) l -- right->left (foldR) =>right graph
        go acc (Connect l r)=foldr f (foldr f acc r) l  
...
* Asupra subgrafurilor se aplică numai funcția edges sau și alte funcții?
  => nu deoarece pt cazul connect am folosit si functia nodes
* Funcția edges însăși se aplică numai asupra subgrafurilor sau și asupra
  altor parametri?
 => folosesc si alti parametrii : nodes left si nodes right 
...
DERIVARE: ...
 caz 1: graful e gol -> ( zero - noduri , zero - muchii)
 caz 2: un nod -> ( nodul , zero- muchii )
 caz 3: overlay l r -> 
       calculez pt l ->(noduri , muchii )
       calulez pt r -> (noduri, muchii )
         => iau toate nodurile : stanga si dreapta  |
         => iau toate muchiile stanga si dreapta    | -> si pun toate astea la un loc
 caz 4: connect l r -> 
       calculez pt l ->(noduri , muchii )
       calulez pt r -> (noduri, muchii )
         => iau toate nodurile : stanga si dreapta    |
         => iau toate muchiile stanga si dreapta      | -> si pun toate astea la un loc
            si muchiile in plus ( din prod cartezian )|

-}
nodesEdges :: Ord a => AlgebraicGraph a -> (Set a, Set (a, a))
--nodesEdges graph = 
nodesEdges Empty = ( Set.empty, Set.empty)
nodesEdges (Node x) = ( Set.singleton x, Set.empty)
nodesEdges (Overlay l r) = 
  let
    (nodesLeft, edgesLeft) = nodesEdges l
    (nodesRight, edgesRight) = nodesEdges r
  in 
    ( Set.union nodesLeft nodesRight, Set.union edgesLeft edgesRight )
...
nodesEdges (Connect l r) = 
   let
    ...
    (nodesRight, edgesRight) = nodesEdges r
   in 
   ( Set.union nodesLeft nodesRight,
     Set.union 
      (Set.union edgesLeft edgesRight) 
      (Set.cartesianProduct nodesLeft nodesRight))
...
foldAlgebraicGraph folder = go  -- parcurgere graf 
  where
...
nodes = foldAlgebraicGraph nodesFolder -- extrage toate nodurile din graf, aplic ce e in nodesFolder pe graf

nodesFolder :: Ord a => AlgebraicGraphFolder a (Set a) (Set a) --mecanism generic de reducere
...
isNode = foldAlgebraicGraph . isNodeFolder -- isNode x graph -> foldAlgebraicGraph (isNodeFolder x) graph
