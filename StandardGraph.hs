...
outNeighbors from graph = Set.foldl (\acc (x,y)-> if x==from then Set.insert y acc else acc ) Set.empty (edges graph) 
...
inNeighbors to graph = Set.foldl (\acc (x,y)-> if y==to then Set.insert x acc else acc ) Set.empty (edges graph) 
...
removeNode node graph =
                        (Set.foldl (\acc x-> if x==node then acc else Set.insert x acc ) 
                            Set.empty 
                            (nodes graph) 
                        ,Set.foldl (\acc (x,y)-> if x/=node && y/=node then Set.insert (x,y) acc else acc ) 
                            Set.empty 
                            (edges graph) )
...
splitNode old news graph = 
                           (Set.foldl (\acc x-> if x==old   -- e cel vechi 
                                                then acc    -- dont add
                                                else Set.insert x acc )  -- add
                                news --replacement nodes
                                (nodes graph) 
                           ,Set.union
                                (Set.union
                                    (Set.cartesianProduct (inNeighbors old graph) news)
                                    (Set.cartesianProduct news (outNeighbors old graph)) )
                                (edges (removeNode old graph))    )
...
mergeNodes prop node graph = 
                        let kick = Set.filter prop (nodes graph)
                            in
                            (Set.foldl (\acc x-> if prop x       -- respecta proprietatea 
                                                    then  acc    -- dont add
                                                    else Set.insert x acc )  -- add
                                    ( if Set.null kick -- daca nu am nimic de scos 
                                      then Set.empty   -- iau nimic
                                      else Set.insert node Set.empty ) -- iau nodul nou 
                                    (nodes graph) 
                            ,Set.union
                                    (Set.union  -- in & out 
                                        (Set.cartesianProduct --muchii in 
                                            (Set.foldl (\acc x-> Set.union acc (Set.filter (not . prop ) (inNeighbors x graph))) Set.empty kick) (Set.singleton node) )      
                                        (Set.cartesianProduct --muchii out 
                                            (Set.singleton node) (Set.foldl (\acc x-> Set.union acc (Set.filter (not . prop ) (outNeighbors x graph))) Set.empty kick) ))
                                    (Set.union   --rest muchii cele care nu au luat vreodata contact cu nodurile 
                                        (Set.foldl (\acc (x,y) -> if prop x || prop y 
                                                            then acc 
                                                            else Set.insert (x,y) acc ) 
                                            Set.empty 
                                            (edges graph) ) 
                                        (Set.foldl (\acc (x,y) -> if prop x && prop y     -- scot 2 si 3 cu muchia 2->3 
                                                            then  Set.insert (node , node) acc  -- inlocuiesc cu nodul 4 cu muchia 4->4
                                                            else acc) 
                                            Set.empty 
                                            (edges graph) ) )
                                    )  
